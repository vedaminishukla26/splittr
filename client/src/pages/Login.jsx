...
const Login = () => {
    ...
    const onSubmit = async (data) => {
        ...
        try {
            ...
            setServerError(error.response?.data?.message || 'Login ')
        }
    }
    ...
    return (
      ...
      <div className="max-w-md w-full bg-white rounded-xl shadow-lg p-8 border border-gray-100">
        <div className="text-center mb-8">
          ...
          <h2 className="text-2xl font-bold text-gray-900">{strings.welcomeBack}</h2>
          <p className="text-gray-500 mt-2">{strings.signIn}</p>
        </div>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
          ...
          >
            {isLoading ? strings.signingIn : strings.signIn}
          </button>
        </form>
